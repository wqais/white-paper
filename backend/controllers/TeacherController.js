...
module.exports.registerTeacher = async (req, res) => {
  ...
  connection.query(
    ...
    async (err, response) => {
      ...
      console.log("Response if email exists : ", response);
      ...
      if (response && response.length > 0) {
        ...
        const data = { ...req.body, password: hashedPwd }; // Create a copy of the data and hash the password
        ...
        connection.query("INSERT INTO teacher SET ?", data, (err, response) => {
          if (err) {
            ...
            return res
              .status(500)
              .json({ msg: "Error registering the teacher." });
          } else {
            console.log("Insert Response: ", response);
            const resp = JSON.parse(JSON.stringify(response)); // Parse the response
            const token = jwt.sign(resp, secretKey); // Generate token
            return res
              .status(200)
              .json({ res: token, msg: "Registered Successfully!!" });
          }
        });
      }
    }
...
module.exports.loginTeacher = async (req, res) => {
  ...
  connection.query(
    ...
    async (err, response) => {
      ...
      if (response && response.length > 0) {
        const teacher = response[0]; // Get the first teacher record
        const pwdCheck = await bcrypt.compare(password, teacher.password); // Compare passwords

        if (pwdCheck) {
          const resData = JSON.parse(JSON.stringify(teacher)); // Parse the response data
          // When creating the token, ensure you're including teacher's ID
          ...
          return res
            .status(200)
            .json({ msg: "Logged in successfully!!", data: token });
        } else {
...
module.exports.getStudentsBySubject = async (req, res) => {
  ...
  connection.query(quer, async (err, response) => {
    ...
    if (response) {
      ...
      return res
        .status(203)
        .json({ res: response, msg: "No Students  Found!" });
    }
  });
};
...
module.exports.updateAttendance = async (req, res) => {
  ...
  connection.query(quer, async (err, response) => {
    ...
    if (response) {
      ...
      return res
        .status(203)
        .json({ res: response, msg: "Oops !! Cannot Update" });
    }
  });
};
